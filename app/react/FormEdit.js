...
function AddQuestion(props) {
  ...
  return (
    <select className="edit-add-question" onChange={onChange}>
      <option onClick={onCancel} value="cancel">
        Add question here...
      </option>
  ...
  return (
    <legend>
      ...
      <input
        ...
        className="edit-legend-mandatory-checkbox"
      />
    </legend>
  );
}
  ...
  return (
    <fieldset>
      ...
      <textarea
        ...
        className="edit-text-input-header"
      />
      <p>Smaller text (optional):</p>
      <textarea
        rows="4"
        value={smallText}
        onChange={editSmallText}
  ...
  return (
    <div>
      ...
      <input
        ...
        value={props.value}
        onChange={changeText}
        className="edit-radio-box-alternative"
      ></input>
      <input
        type="button"
        value="❌"
        onClick={() => props.changeAlternative("delete")}
      ></input>
      <input
        type="button"
        value="⇓"
        onClick={() => props.changeAlternative("moveUp")}
      ></input>
      <input
        type="button"
        value="⇑"
        onClick={() => props.changeAlternative("moveDown")}
      ></input>
    </div>
  );
}
...
function RadioBoxesFormElem(props) {
  ...
  function changeAlternative(type, data, i) {
    switch (type) {
      case "delete":
        props.changeQuestion("edit", {
          ...q,
          alternatives: alternatives.deleteElem(i),
        });
        break;
      case "add":
      ...
      case "edit":
        props.changeQuestion("edit", {
          ...q,
          alternatives: alternatives.replaceElem(i, data),
        });
        break;
      case "moveUp":
        moveUp(i);
  ...
  let radioBoxes = alternatives.map((value, i) => {
    ...
    return (
      <RadioBoxElem
        ...
        value={value}
        id={id}
        changeAlternative={(type, data) => changeAlternative(type, data, i)}
      />
    );
  });
  ...
  return (
    <fieldset>
      ...
      <textarea
        ...
        value={question}
        onChange={editQuestion}
        className="edit-radio-box-header"
      />
      <br />
      <input
        type="button"
        value="➕"
        onClick={() => changeAlternative("add", null, -1)}
      ></input>
      {radioBoxes}
      <input
        type="button"
        value="➕"
        onClick={() => changeAlternative("add", null, radioBoxes.length - 1)}
      ></input>
    </fieldset>
  );
}
...
export function Form(props) {
  ...
  function addQuestion(data, i) {
    ...
    let newQuestions = [...questions.slice(0, i), elem, ...questions.slice(i)];
    renumerateAndSet(newQuestions);
  }

  function renumerateAndSet(newQuestions) {
    setQuestions(
      newQuestions.map((elem, index) => {
  ...
  function changeQuestion(type, data, i) {
    switch (type) {
      ...
      case "edit":
        setQuestions(
          questions.replaceElem(i, { ...questions[i], value: data })
        );
        break;
      case "moveUp":
        moveUp(i);
      ...
      case "setMandatory":
        setQuestions(
          questions.replaceElem(i, { ...questions[i], mandatory: data })
        );
        break;
    }
  }
  ...
  let formElements = questions.map((question, i) => {
    ...
    if (question.type === 0) {
      q = (
        <TextInputFormElem
          question={question}
          changeQuestion={(type, data) => changeQuestion(type, data, i)}
        />
      );
    ...
    return (
      <div key={i} className="edit-q-container">
        ...
        <div className="edit-right-buttons">
          <input
            className="edit-question-up"
            type="button"
            value="⇑"
            onClick={() => changeQuestion("moveDown", null, i)}
          ></input>
          <input
            className="edit-question-delete"
            type="button"
            value="❌"
            onClick={() => changeQuestion("delete", null, i)}
          ></input>
          <input
            className="edit-question-down"
            type="button"
            value="⇓"
            onClick={() => changeQuestion("moveUp", null, i)}
          ></input>
        </div>
        {q}
      </div>
  ...
  let cancelSave = (
    <div className="edit-cancel-save">
      <input
        type="button"
        value="Save"
        onClick={() => props.submitNewForm(questions)}
      />
      <a href="/info">
  ...
  return (
    <form>
      ...
      <AddQuestion
        ...
        changeQuestion={(type, data) =>
          changeQuestion(type, data, formElements.length)
        }
      />
      <br />
      <br />
