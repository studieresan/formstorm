...
export function FormWrapperFill(props) {
  ...
  function fetchUserData(internal_id, prepost) {
    fetch(`/api/form/get-user-data?internal_id=${internal_id}&prepost=${prepost}`)
      .then(getFetchErr('json'))
      .then((data) => {
        if (prepost === '0')
          var formTypeId = data.event.pre_form;
        else
          var formTypeId = data.event.post_form;
  ...
  let sendButton = (state.state === 2)
    ? <SendButton answers={state.answers}
                  internal_id={props.internal_id}
                  prepost={props.prepost}
                  sendFunc={submitAnswers}/>
    : '';

  let form = (state.state === 1 || state.state === 2)
  ...
  let submitSuccess = (state.state === 3)
    ? <SubmitSuccess />
    : '';

  let alreadySubmitted = (state.state === 4)
    ? <AlreadySubmitted />
    : '';
